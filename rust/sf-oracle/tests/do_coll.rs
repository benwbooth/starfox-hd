...
fn do_coll_vs_rom() {
    ...
    for (cc, hp, ap, tun) in cases {
        ...
        if o != c { cur_diffs += 1; }
        if o != f { fix_diffs += 1; }
        eprintln!("cc={cc} hp={hp:#04x} ap={ap} tun={tun}: ROM{o:?} cur{c:?}{} fixed{f:?}{}",
            if o == c { "" } else { " <DIFF>" }, if o == f { "" } else { " <DIFF>" });
    }
    eprintln!("current port: {cur_diffs} diffs vs ROM; faithful fix: {fix_diffs} diffs");
    assert!(cur_diffs > 0, "expected the current port to deviate (documents the bug)");
