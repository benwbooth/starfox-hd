...
//! These variants create one actor and install an already lowered path. The
//! player mesh/auxiliary variants are deliberately not accepted here.
...
use super::{ObjectId, ObjectSpawnDefaults, ObjectStore, PathCursor, RandomState, OBJECT_CAPACITY};
...
pub fn launch(
    ...
    let hostile = !player_heavy && request.weapon != PathWeapon::VariantGuided;
    ...
    let Some(created) =
        weapon_creation::common(objects, caller, request.parameters, request.defaults)
