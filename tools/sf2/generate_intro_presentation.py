#!/usr/bin/env python3
"""Generate the native SF2 boot and title-attract presentation.

The inputs are the unattended retail attract cycle and accepted-Start response
captured by ``mesen_boot_presentation.lua``. The output contains ordinary
palette-indexed image deltas for typed native intro state. It contains no
...
MESEN_TOP_BORDER = 8
RETAIL_FRAME_STEP = 4
FIRST_RETAIL_FRAME = 4
FRAME_COUNT = 1_140
TITLE_RESPONSE_FIRST_RETAIL_FRAME = 3_124
TITLE_RESPONSE_FRAME_COUNT = 5
ASSET_MAGIC = b"SFIN2"
...
def main() -> None:
    ...
    parser.add_argument("--title-response-dir", type=Path, required=True)
    ...
    def append_track(directory: Path, first_frame: int, frame_count: int) -> None:
        nonlocal changed_pixels
        previous = [0] * (FRAME_WIDTH * FRAME_HEIGHT)
        for index in range(frame_count):
            frame = first_frame + index * RETAIL_FRAME_STEP
            image = read_ppm(directory / f"sf2_boot_{frame:04d}.ppm")
            current = []
            for offset in range(0, len(image), 3):
                ...
                if palette_index is None:
                    ...
                    if palette_index > 65_535:
                        raise SystemExit("intro presentation needs more than 65536 colors")
    ...
    append_track(
        args.title_response_dir,
        TITLE_RESPONSE_FIRST_RETAIL_FRAME,
    ...
    print(
        ...
        f"{FRAME_COUNT + TITLE_RESPONSE_FRAME_COUNT} frames, "
        f"{changed_pixels} changes, "
