-- Capture pre-instruction Super FX state for the first $01:CD99 render job.
-- This is intentionally a diagnostic oracle: it stops as soon as that job's
-- STOP enters the pipeline, keeping the trace small enough for exact diffing.
...
local function trace(address)
  ...
  fields[#fields + 1] = string.format("%02X", state["cart.coprocessor.romReadBuffer"] or 0)
  fields[#fields + 1] = string.format("%02X", state["cart.coprocessor.ramDelay"] or 0)
  fields[#fields + 1] = string.format("%04X", state["cart.coprocessor.ramWriteAddress"] or 0)
  fields[#fields + 1] = string.format("%02X", state["cart.coprocessor.ramWriteValue"] or 0)
...
local function stop()
  ...
  local file = assert(io.open(
    emu.getScriptDataFolder() .. "/cd99-register-trace.txt", "wb"))
  file:write(table.concat(lines))
  file:close()
  emu.log("SF2_CD99_REGISTER_TRACE_DONE")
  emu.stop(0)
end
