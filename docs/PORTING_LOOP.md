...
- [x] SF1 retail/native opening through the Corneria launch-submap return — extend the shared physical-input trace to global tick 1079; compare background, game frame, player depth delta, active slots, flat shape identities, and all live positions from initialized gameplay onward. Fix `viewopening`'s pre-chase carry branch, player/camera list order, player-opening boost fallthrough, and boost-flame initializer/child scheduling. The post-return fade's storage-only countdown is normalized explicitly; all gameplay fields stay strict. Focused boost/player traces and the 1,080-tick whole-machine gate pass. Post-fade continuation remains a separate synchronization task.
