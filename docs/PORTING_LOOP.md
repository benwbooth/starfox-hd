...
- [ ] SF1 retail/native opening into active Corneria flight — strict parity is
  currently certified only through global tick 1063. The former 1,200-tick
  claim used map-countdown normalization and a later retail-state/RNG
  substitution window; both are removed. Resume from the exact tick-1064
  divergence and advance the certified boundary without adapter exceptions.
