...
- [x] Shark + fzaco + hardenemy1/hard90yrfog + zaco3_strat — `shark{,_cont,_cont2,a}_*` mine-drop climb; `fzaco{,2,3,_cont,_cont2}_*` brake→aim→climb; hard-object initialization and live fog visibility; `zaco3_strat` alias; tests `shark_fzaco.rs` and `fog_visibility.rs`.
...
- [x] Jump0/1 + sokuten + item3/6 + core0/1 + rightwall/mine1 + fog — `jump{0,1,0a}_*` hop→HMISSILE1; `sokuten_*` heading turn; `item3` +5 body HP / public `item6` wireship; `core{0,1}_*` gasf_flag1 flash; inert `rightwall`/`mine1`; exact fog shape hide/restore; tests `jump_sokuten_item3.rs` and `fog_visibility.rs`.
...
- [x] SF1 fog-object visibility — restore KSTRATS `s_initfog`/`s_dofog` as ordinary retained shape state; use the inclusive 2,000-unit Z cutoff for hard scenery, tanks, turrets, and pillars; keep the map fog flag's early-exit semantics; and give `hard180YRfog` its distinct non-enemy-collision initializer instead of aliasing ordinary `hard180YR`. Exact boundary, restoration, collision, family, table, route, coexec, and architecture regressions pass.
...
- [x] SF1 retail/native opening into active Corneria flight — extend the shared physical-input trace through global tick 1199; compare background, game frame, player forward velocity and depth delta, active slots, flat shape identities, all live positions, and the launch wingmates' departure counters. Fix the two-stage scramble wipe, bounded retail sound-upload pause, player/path/friend initializer fallthrough, player/camera/list scheduling, view-speed chase, boost-flame scheduling, and exact friend removal boundary. Normalize the map countdown only during the proven tick-1064-through-1078 storage window; later state is strict again. Focused regressions and the 1,200-tick whole-machine gate pass.
