...
| Meteor Queen Dragoon encounter | Four hash-bound saved-state and callback replays, `tools/sf2/fixtures/meteor_queen_dragoon.trace` | One body and four linked components, durability, destruction shapes, and actor movement callbacks are verified. A mission-state-unforced replay naturally defeats Queen Dragoon, follows its dropped switch, reduces the objective mirrors from 2 to 1, opens the installation, and enters Meteor's first interior room. The separate strategic-map return observation still injects zero remaining objectives and proves only that presentation | certain |
