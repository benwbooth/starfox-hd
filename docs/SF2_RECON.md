...
| Venom armored-room progression | Controlled full-durability saved-state replay, `tools/sf2/fixtures/venom_knight_gate.trace` | Placing only the retail player beyond the reactor door transitions `0ADE -> 0AE2 -> 0B45` in ten frames while the Knight remains at durability 100 throughout. The route is position-triggered and does not require defeating the Knight | certain |
| Meteor installation core encounter | Three hash-bound saved-state replays, `tools/sf2/fixtures/meteor_installation_core.trace` | From an isolated saved state, natural right/forward movement arms the dormant core; an accelerated exact-actor hit advances its retail damage/death paths, decrements both occupied-world mirrors from 2 to 1, and leaves Select's four-form Walker-to-Arwing transition intact. The saved-state ancestry used forced base flags, controller durability, and teleports, so this certifies the encounter mechanics but not its unforced campaign route | certain for encounter mechanics; route unknown |
| Meteor Queen Dragoon encounter | Five hash-bound saved-state and callback replays, `tools/sf2/fixtures/meteor_queen_dragoon.trace` | One body and four linked components, durability, destruction shapes, and actor movement callbacks are verified. Mission-state-unforced replays naturally defeat Queen Dragoon, follow its dropped switch, reduce the global occupied-world mirrors from 2 to 1, open and traverse the installation, complete the sortie, and return to the strategic map with one occupied campaign world remaining. The mirrors are campaign-global, not two Meteor-local objectives, so this evidence does not prove that Queen Dragoon and the installation core are separate sorties. The older forced-zero-counter replay proves only the resulting return presentation | certain |
| Fortuna Kick Gunner strategy | Static path extraction plus hash-bound live trace, `tools/sf2/fixtures/fortuna_kick_gunner.trace` | The guardian's eight corner routes, two floor steps, 20-step dive, five 14-step attacks, bob sequences, 20-step retreat, animation frames, and waits are decoded. Each linked mount performs ten settling actions and fires exactly one selector-26 projectile; the projectile has shape catalog 44, speed 20, durability 120, and the decoded wait lifetime. The live trace independently locks observed world poses and mount creation fields | certain |
| Opening-sortie hostile-projectile path | Static path extraction plus compact logic/pose traces, `tools/sf2/fixtures/first_sortie_projectile_logic.trace`, `tools/sf2/fixtures/first_sortie_projectiles.trace` | The retail path sets speed 63, performs three target contractions, immediately faces the selected target, later applies scheduled smooth aim, waits, and continues through the cooperative dispatcher. Independent replay covers all 1,430 retained poses across 43 lifetimes; immediate reuse of one retail object slot at frames 4,820/4,824 is two shots, not one discontinuous trajectory | certain |
| Leon-pressure hostile-projectile path | Static path extraction plus non-perturbing callback replay, `tools/sf2/fixtures/leon_pressure_projectile_logic.trace`, `tools/sf2/fixtures/leon_pressure.trace` | The same decoded hostile-laser path is statically required byte-for-byte. Read-only callbacks preserve all 416 authoritative gameplay records while exposing the semantic operations. Offline replay reproduces all 156 retained poses across five lifetimes; shipping Rust now advances typed projectile flight state and retains no pose-track playback | certain |
