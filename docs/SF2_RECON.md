...
| Meteor Queen Dragoon encounter | Hash-bound sortie and actor callback replays, `tools/sf2/fixtures/meteor_queen_dragoon.trace` | One body and four linked components, durability, destruction shapes, and actor movement callbacks are verified. The strategic-map return observation deliberately injects zero remaining objectives, so it does not prove natural Meteor completion | partial |
